...
from RoMa.roma.utils import get_tuple_transform_ops
from RoMa.roma.utils.local_correlation import local_correlation
from RoMa.roma.utils.utils import cls_to_flow_refine
from RoMa.roma.utils.kde import kde
...
class RegressionMatcher(nn.Module):
    ...
    def to_pixel_coordinates(self, coords, H_A, W_A, H_B, W_B):
        if isinstance(coords, (list, tuple)):
            ...
            kpts_A = torch.stack(((W_A-1)/2 * (kpts_A[...,0]+1), (H_A-1)/2 * (kpts_A[...,1]+1)),axis=-1)
            kpts_B = torch.stack(((W_B-1)/2 * (kpts_B[...,0]+1), (H_B-1)/2 * (kpts_B[...,1]+1)),axis=-1)
