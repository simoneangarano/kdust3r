...
TEST_CRITERION = "ConfLoss(Regr3D(L21, norm_mode='avg_dis'), alpha=0.2) + Regr3D_ScaleShiftInv(L21, gt_scale=True)"
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./log/cs10/', type=str, help="path where to save the output")
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if do_test_now(data_iter_step, accum_iter) and log_writer is not None:
            ...
            for test_name, testset in data_loader_test.items():
                stats = test_one_epoch(model_without_ddp, test_criterion, testset,
                                       device, epoch, log_writer=log_writer, args=args, prefix=test_name,
                                       kd=args.kd, teacher=teacher, features=args.kd, curr_step=epoch_1000x)
                test_stats[test_name] = stats
