...
class DTU(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        while len(imgs_idxs) > 0:  # some images (few) have zero depth
            ...
            impath = osp.join(self.ROOT, 'Rectified', instance, f'rect_{view_idx}.png')

            # load camera params
            input_metadata = np.load(impath.replace('png', 'npz'))
            camera_pose = input_metadata['camera_pose'].astype(np.float32)
            intrinsics = input_metadata['camera_intrinsics'].astype(np.float32)

            # load image and depth
            rgb_image = imread_cv2(impath)
            depthmap = imread_cv2(impath.replace('Rectified', 'Depths').replace('rect', 'depth_map'), cv2.IMREAD_UNCHANGED)
            depthmap = (depthmap.astype(np.float32) / 65535) * np.nan_to_num(input_metadata['maximum_depth'])

            if mask_bg:
                # load object mask
                maskpath = impath.replace('Rectified', 'Depths').replace('rect', 'depth_mask')
            ...
            # load features
            if self.features:
                features = np.load(impath.replace('png', 'npy'))
                points = 0
            else:
                features, points = 0, 0
