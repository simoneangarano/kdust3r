...
def get_parser():
    ...
    parser.add_argument("--output_dir", type=str, default="/ssd1/sa58728/dust3r/data/misc/co3d_processed_xl")
    parser.add_argument("--co3d_dir", type=str, default="/ssd1/sa58728/dust3r/data/misc/co3d")
    parser.add_argument("--num_sequences_per_object", type=int, default=1000)
...
def get_set_list(category_dir, split, is_single_sequence_subset=False):
    if not os.path.exists(osp.join(category_dir, "set_lists.json")):
    ...
    with open(osp.join(category_dir, "set_lists.json")) as f:
        subset_lists_data = json.load(f)
        sequences_all = []
        for k in subset_lists_data.keys():
            if split in k:
                sequences_all.extend(subset_lists_data[k])
    return sequences_all
...
if __name__ == "__main__":
    ...
    for split in ['train', 'test']:
        ...
        for category in categories:
            if not os.path.exists(osp.join(args.co3d_dir, category)):
                print(f"Skipping {category} as it does not exist in {args.co3d_dir}")
                continue
